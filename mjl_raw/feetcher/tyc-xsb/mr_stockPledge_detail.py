...
"""
@Time ： 2020/12/16 5:01 PM
@Auth ： Stone
@File ：mr_stockPledge_detail.py
@IDE  ：PyCharm
"""

# 质押明细详情  1022   mr_stockPledge_detail

# STOCK_PLEDGE_DETAIL(1022,"stockPledgeDetail"),
...
class Mr_StockPledge_detail(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "STOCK_PLEDGE_DETAIL",
                "trdDataProvider": "TIANYANCHA",
                "trdDataRequest": {
                    "businessId": "北京百度网讯科技有限公司",
                    "id":""
                },
                # "companyId": "",
    ...
    def get_company_list(self):
        sql = "select company_name,qyxx_id from company_manager where `status` like '新三板%' "
        try:
    ...
    def get_response(self, businessId, c_id):
        self.post_data['trdDataRequest']['businessId'] = businessId
        self.post_data['trdDataRequest']['id'] = c_id
        time.sleep(0.3)
    ...
    def parse_data(self, response):
        ...
        if result_dict == '':
            ...
            result_frame.append([
                ...
                result_dict.get('proOfAll'),
                result_dict.get('shareHolderId'),
                result_dict.get('reason'),
                result_dict.get('lastValue'),
                result_dict.get('companyCount'),
                result_dict.get('pledgeInsId'),
                result_dict.get('code'),
                result_dict.get('pledgeInsLogo'),
                result_dict.get('endDate'),
                result_dict.get('dayClosePos'),
                result_dict.get('pledgeAmount'),
                result_dict.get('pledgeInsType'),
                result_dict.get('businessId'),
                result_dict.get('shareHolder'),
                result_dict.get('proOfSelf'),
                result_dict.get('annDate'),
                result_dict.get('shareHolderType'),
                result_dict.get('pledgeInsAlias'),
                result_dict.get('pledgeIns'),
                result_dict.get('graphId'),
                result_dict.get('startDate'),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'mr_stockPledge_detail.csv'
    ...
    def run_read_csv(self):
        hive_data_frame = pd.DataFrame()
        # 需要9和14  .drop_duplicates()  去重
        df = pd.read_csv('mr_stockPledge_detailList.csv', usecols=[9, 14])
        df_li = df.values.tolist()
        count = 0
        for res in df_li:
            # print(res[0],res[1])
            count += 1
            print("==========正在爬取第{}个==========".format(count))
            resp_json = self.get_response(res[0],res[1])
            # print(resp_json)
            hive_data = self.parse_data(resp_json)
            # print(hive_data)
            hive_data_frame = hive_data_frame.append(hive_data)
...
if __name__ == '__main__':
    mr_stockPledge_detail = Mr_StockPledge_detail()
    mr_stockPledge_detail.run_read_csv()
