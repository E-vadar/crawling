...
"""
@Time ： 2020/12/16 6:11 PM
@Auth ： Stone
@File ：creditchinahistoryInfo.py
@IDE  ：PyCharm
"""

# 历史行政处罚-其他来源   865

# CREDIT_CHINA(865,"creditChina"),
...
class CreditChinaHistoryInfo(object):
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('result', result),
                    self.deal_json('areaName', result),
                    self.deal_json('punishNumber', result),
                    self.deal_json('reason', result),
                    self.deal_json('typeSecond', result),
                    self.deal_json('evidence', result),
                    self.deal_json('punishStatus', result),
                    self.deal_json('decisionDate', result),
                    self.deal_json('type', result),
                    self.deal_json('departmentName', result),
                    self.deal_json('punishName', result),
...
if __name__ == '__main__':
    credit_china_history_info = CreditChinaHistoryInfo()
    credit_china_history_info.run_main()