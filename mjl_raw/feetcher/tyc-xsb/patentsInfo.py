        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('title', result),
                    self.deal_json('patentNum', result),
                    self.deal_json('uuid', result),
                    self.deal_json('pubnumber', result),
                    self.deal_json('applicationTime', result),
                    self.deal_json('applicantname', result),
                    self.deal_json('eventTime', result),
                    self.deal_json('inventor', result),
                    self.deal_json('id', result),
                    self.deal_json('address', result),
                    self.deal_json('agency', result),
                    self.deal_json('abstracts', result).replace('，', '#'),
                    self.deal_json('searchType', result),
                    self.deal_json('connList', result),
                    self.deal_json('_type', result),
                    self.deal_json('lawStatus', result),

                    self.deal_json('cat', result),
                    self.deal_json('applicantName', result),
                    self.deal_json('pubDate', result),
                    self.deal_json('applicationPublishTime', result),
                    self.deal_json('appnumber', result),
                    self.deal_json('patentType', result),
                    self.deal_json('imgUrl', result),
                    self.deal_json('uni', result),
                    self.deal_json('mainCatNum', result),
                    self.deal_json('createTime', result),
                    self.deal_json('patentName', result),
                    self.deal_json('applicationPublishNum', result),
                    self.deal_json('allCatNum', result),
...
if __name__ == '__main__':
    patents_info = PatentsInfo()
    patents_info.run_main()