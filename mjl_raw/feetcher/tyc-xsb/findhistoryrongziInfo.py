...
"""
@Time ： 2020/12/16 9:17 AM
@Auth ： Stone
@File ：findhistoryrongziInfo.py
@IDE  ：PyCharm
"""
# 融资历史---826
...
class Find_history_rongzi_Info(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "FINDHITORYRONGZI_INFO",
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('money', result),
                    self.deal_json('newsUrl', result),
                    self.deal_json('pubTime', result),
                    self.deal_json('companyName', result),
                    self.deal_json('newsTitle', result),
                    self.deal_json('investorName', result),
                    self.deal_json('share', result),
                    self.deal_json('value', result),
                    self.deal_json('date', result),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'findhistoryrongziInfo.csv'
...
if __name__ == '__main__':
    find_history_rongzi_info = Find_history_rongzi_Info()
    find_history_rongzi_info.run_main()
