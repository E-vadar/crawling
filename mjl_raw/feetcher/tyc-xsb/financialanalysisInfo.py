...
"""
@Time ： 2020/12/16 11:40 AM
@Auth ： Stone
@File ：financialanalysisInfo.py
@IDE  ：PyCharm
"""
...
class FinancialanalysisInfo(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "FINANCIALANALYSIS_INFO",
    ...
    def get_response(self, company_name, qyxx_id=None, pageNum = 1):
        self.post_data['trdDataRequest']['name'] = "平安银行股份有限公司"
        self.post_data['trdDataRequest']['pageNum'] = pageNum
        self.post_data['companyName'] = "平安银行股份有限公司"
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            # 重新组合字典，进行导出
            result_dict['noSqlId'] = response.get('data').get('noSqlId'),
            result_dict['qyxx_id'] = qyxx_id
            result_dict['company_name'] = company_name
            result_dict['spiders_time'] = str(datetime.now().strftime("%Y-%m-%d")),

            # print(result_dict)
            return result_dict

    def mk_dir_folder(self, folder_name):
    ...
    def get_filename(self):
        ...
        ben_filename = self.mk_dir_folder(filename)
        return ben_filename

    def save_to_json(self, json_name, data_dict):
        try:
            with open(json_name, 'w', encoding='utf-8') as f:
                f.write(json.dumps(data_dict, ensure_ascii=False))
                print("{}文件保存成功...".format(json_name))
        except Exception as err:
            print("保存json出错，错误的原因是{}".format(err))
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'FinancialanalysisInfo.csv'
        ...
        for com in com_list:
            ...
            if page_number != 0:
                for page in range(1, int(page_number) + 1):
                    ...
                    folder_path = self.get_filename()
                    file_path = os.path.join(folder_path, str(com[0])+'.json')
                    self.save_to_json(file_path, hive_data)
                    # print(hive_data)
                    #
...
if __name__ == '__main__':
    financialanalysis_Info = FinancialanalysisInfo()
    financialanalysis_Info.run_main()