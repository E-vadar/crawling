...
"""
@Time ： 2020/12/19 6:01 PM
@Auth ： Stone
@File ：dishonestInfo.py
@IDE  ：PyCharm
"""

# DISHONEST_INFO(843,"dishonestInfo"), 失信人
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                staff_list = self.deal_json('staff', result),
                for staff in staff_list:
                    result_frame.append([
                        ...
                        self.deal_json('businessentity', result),
                        self.deal_json('areaname', result),
                        self.deal_json('legelType', result),
                        self.deal_json('legelId', result),
                        self.deal_json('courtname', result),
                        self.deal_json('unperformPart', result),

                        # self.deal_json('staff', result),    # 列表
                        self.deal_json('role', staff),
                        self.deal_json('code', staff),
                        self.deal_json('name', staff),

                        self.deal_json('cpwsUrl', result),
                        self.deal_json('type', result),
                        self.deal_json('performedPart', result),
                        self.deal_json('iname', result),
                        self.deal_json('disrupttypename', result).replace('\n', '').replace('\n\r', '').replace(
                            '\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n',
                                                                                                          '').replace(
                            ',', '#').replace('，', '#'),
                        self.deal_json('casecode', result),
                        self.deal_json('cardnum', result),
                        self.deal_json('performance', result),
                        self.deal_json('legelToco', result),
                        self.deal_json('regdate', result),
                        self.deal_json('publishdate', result),
                        self.deal_json('gistunit', result),
                        self.deal_json('duty', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n',
                                                                                                     '').replace('\r',
                                                                                                                 '').replace(
                            '\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
                        self.deal_json('id', result),
                        self.deal_json('gistid', result),
                        self.deal_json('cid', result),
...
if __name__ == '__main__':
    dishonest_info = DishonestInfo()
    dishonest_info.run_main()