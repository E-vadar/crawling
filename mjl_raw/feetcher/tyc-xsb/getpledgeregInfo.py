...
"""
@Time ： 2020/12/16 5:07 PM
@Auth ： Stone
@File ：getpledgeregInfo.py
@IDE  ：PyCharm
"""

# GETPLEDGEREG_INFO(795,"getpledgeregInfo"),  知识产权出质
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('pledgeeName', result),
                    self.deal_json('pledgeRegPeriod', result),
                    self.deal_json('pledgeeType', result),
                    self.deal_json('iprType', result),
                    self.deal_json('pledgeId', result),
                    self.deal_json('pledgorType', result),
                    self.deal_json('iprCertificateNum', result),
                    self.deal_json('state', result),
                    self.deal_json('pledgeeHid', result),
                    self.deal_json('detailList', result),
                    self.deal_json('pledgorName', result),
                    self.deal_json('id', result),
                    self.deal_json('pledgeeCid', result),
                    self.deal_json('iprName', result),
                    self.deal_json('pledgorCid', result),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'getpledgeregInfo.csv'
...
if __name__ == '__main__':
    get_pledge_reg_info = GetpLedgeRegInfo()
    get_pledge_reg_info.run_main()