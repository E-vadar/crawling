...
"""
@Time ： 2020/12/16 5:12 PM
@Auth ： Stone
@File ：mortgageInfo.py
@IDE  ：PyCharm
"""

# MORTGAGE_INFO(844,"mortgageInfo"),  动产抵押
...
class MortgageInfo(object):
    ...
    def get_response(self, company_name, qyxx_id=None, pageNum = 1):
        self.post_data['trdDataRequest']['name'] = "讷河市丰盛现代农业农机专业合作社"
        self.post_data['trdDataRequest']['pageNum'] = pageNum
        self.post_data['companyName'] = "讷河市丰盛现代农业农机专业合作社"
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                ...
                result_frame.append([
                    ...
                    self.deal_json('overviewAmount', base_info),  # 只解析baseInfo字段数据
                    self.deal_json('scope', base_info),
                    self.deal_json('status', base_info),
                    self.deal_json('remark', base_info),
                    self.deal_json('regDate', base_info),
                    self.deal_json('overviewType', base_info),
                    self.deal_json('type', base_info),
                    self.deal_json('cancelReason', base_info),
                    self.deal_json('overviewScope', base_info),
                    self.deal_json('id', base_info),
                    self.deal_json('amount', base_info),
                    self.deal_json('overviewRemark', base_info),
                    self.deal_json('overviewTerm', base_info),
                    self.deal_json('regDepartment', base_info),
                    self.deal_json('regNum', base_info),
                    self.deal_json('term', base_info),
                    self.deal_json('base', base_info),
                    self.deal_json('cancelDate', base_info),
                    self.deal_json('publishDate', base_info),

                    # result['changeInfoList'],  # 列表
                    # result['pawnInfoList'],   # 列表
                    # result['peopleInfo'],    # 列表
