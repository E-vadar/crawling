...
"""
@Time ： 2020/12/16 2:36 PM
@Auth ： Stone
@File ：purchaseLand.py
@IDE  ：PyCharm
"""

# 购地信息---885   purchaseLand
# PURCHASE_LAND(885,"purchaseLand"),
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('projectLocation', result),
                    self.deal_json('area', result),
                    self.deal_json('contractDate', result),
                    self.deal_json('landUseType', result),
                    self.deal_json('contractedVolumeRate', result),
                    self.deal_json('transactionPrice', result),
                    self.deal_json('instalmentPayment', result), # 列表
                    self.deal_json('contractedVolumeRateCeiling', result),
                    self.deal_json('electronicRegulatoryNumber', result),
                    self.deal_json('committedTime', result),
                    self.deal_json('landSupplyMethod', result),
                    self.deal_json('scheduledCompletion', result),
                    self.deal_json('landUsePeriod', result),
                    self.deal_json('landUseRightPerson', result),
                    self.deal_json('companyList', result),  # 列表
                    self.deal_json('chainLink', result),
                    self.deal_json('authority', result),
                    self.deal_json('district', result),
                    self.deal_json('landLevel', result),
                    self.deal_json('category', result),
                    self.deal_json('projectName', result),
                    self.deal_json('agreementStartTime', result),
                    self.deal_json('landSourceView', result),
