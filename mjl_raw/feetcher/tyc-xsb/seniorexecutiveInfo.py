...
"""
@Time ： 2020/12/16 11:16 AM
@Auth ： Stone
@File ：seniorexecutiveInfo.py
@IDE  ：PyCharm
"""

# 高管信息---855   SENIOREXECUTIVE_INFO
...
class SeniorexecutiveInfo(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "SENIOREXECUTIVE_INFO",
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('resume', result).replace(',', '#'),
                    self.deal_json('education', result),
                    self.deal_json('cType', result),
                    self.deal_json('numberOfShares', result),
                    self.deal_json('sex', result),
                    self.deal_json('salary', result),
                    self.deal_json('managerGroup', result),
                    self.deal_json('reportDate', result),
                    self.deal_json('name', result),
                    self.deal_json('term', result),
                    self.deal_json('position', result),
                    self.deal_json('graphId', result),
                    # self.deal_json('age', result),
                ])
            return result_frame
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'seniorexecutiveInfo.csv'
...
if __name__ == '__main__':
    seniorexecutive_info = SeniorexecutiveInfo()
    seniorexecutive_info.run_main()
