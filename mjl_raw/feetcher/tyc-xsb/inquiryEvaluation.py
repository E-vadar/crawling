...
"""
@Time ： 2020/12/16 5:29 PM
@Auth ： Stone
@File ：inquiryEvaluation.py
@IDE  ：PyCharm
"""

# 询价评估   1023
# INQUIRY_EVALUATION(1023,"inquiryEvaluation"),
...
class InquiryEvaluation(object):
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                ...
                for eva in self.deal_json('evaluationAgency', result):
                    result_frame.append([
                        ...
                        self.deal_json('insertTime', result),
                        self.deal_json('subjectname', result),
                        self.deal_json('ename', result),
                        self.deal_json('caseNumber', result),
                        # self.deal_json('evaluationAgency', result),# 列表
                        self.deal_json('orgGid', eva),
                        self.deal_json('orgName', eva),

                        self.deal_json('execCourtName', result),
                        self.deal_json('enameGid', result),
                        self.deal_json('subjectType', result),
                        self.deal_json('enameType', result),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'inquiryEvaluation.csv'
