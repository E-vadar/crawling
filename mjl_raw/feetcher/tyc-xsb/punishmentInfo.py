...
"""
@Time ： 2020/11/23 11:47 AM
@Auth ： Stone
@File ：punishmentInfo.py.py
@IDE  ：PyCharm
"""
# 公司被处罚次数  847   PUNISHMENT_INFO(847,"punishmentInfo"),
...
class Punishment_info(object):
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        result_frame = []
        if result_dict == '':
            print("{}--此公司没有被处罚次数".format(company_name))
        else:
            # print("舆情有内容，开始格式化！")
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('departmentName', result),
                    self.deal_json('publishDate', result),
                    self.deal_json('description', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
                    self.deal_json('type', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
                    self.deal_json('content', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
                    self.deal_json('decisionDate', result),
                    self.deal_json('legalPersonName', result),
                    self.deal_json('punishNumber', result),
                    self.deal_json('regNum', result),
                    self.deal_json('name', result),
                    self.deal_json('base', result),
                ])
            return result_frame
...
if __name__ == '__main__':
    punishment_info = Punishment_info()
    punishment_info.run_main()