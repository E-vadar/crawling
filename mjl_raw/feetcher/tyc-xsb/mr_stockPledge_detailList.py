...
"""
@Time ： 2020/12/16 4:57 PM
@Auth ： Stone
@File ：mr_stockPledge_detailList.py
@IDE  ：PyCharm
"""

# 质押明细    1021    mr_stockPledge_detailList
# STOCK_PLEDGE_DETAIL_LIST(1021,"stockPledgeDetailList"),
...
class Mr_StockPledge_DetailList(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "STOCK_PLEDGE_DETAIL_LIST",
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('shareHolderId', result),
                    self.deal_json('lastValue', result),
                    self.deal_json('companyCount', result),
                    self.deal_json('endDate', result),
                    self.deal_json('pledgeAmount', result),
                    self.deal_json('businessId', result),
                    self.deal_json('shareHolder', result),
                    self.deal_json('proOfSelf', result),
                    self.deal_json('annDate', result),
                    self.deal_json('shareHolderType', result),
                    self.deal_json('graphId', result),
                    self.deal_json('startDate', result),
                    self.deal_json('status', result),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'mr_stockPledge_detailList.csv'
...
if __name__ == '__main__':
    mr_stockPledge_detailList = Mr_StockPledge_DetailList()
    mr_stockPledge_detailList.run_main()