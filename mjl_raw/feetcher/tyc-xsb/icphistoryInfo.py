...
"""
@Time ： 2020/12/16 3:42 PM
@Auth ： Stone
@File ：icphistoryInfo.py
@IDE  ：PyCharm
"""
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                name_list = self.deal_json('webSite', result)
                result_frame.append([
                    ...
                    self.deal_json('examineDate', result),
                    self.deal_json('companyType', result),
                    self.deal_json('webName', result),
                    self.deal_json('ym', result),
                    self.deal_json('liscense', result),
...
if __name__ == '__main__':
    icp_history_info = IcphistoryInfo()
    icp_history_info.run_main()