...
"""
@Time ： 2020/12/16 5:37 PM
@Auth ： Stone
@File ：owntaxInfo.py
@IDE  ：PyCharm
"""

# OWNTAX_INFO(851,"owntaxInfo"),  欠税公告
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('taxIdNumber', result),
                    self.deal_json('newOwnTaxBalance', result),
                    self.deal_json('ownTaxAmount', result),
                    self.deal_json('publishDate', result),
                    self.deal_json('ownTaxBalance', result),
                    self.deal_json('type', result),
                    self.deal_json('personIdNumber', result),
                    self.deal_json('taxCategory', result),
                    self.deal_json('taxpayerType', result),
                    self.deal_json('personIdName', result),
                    self.deal_json('name', result),
                    self.deal_json('location', result),
                    self.deal_json('department', result),
                    self.deal_json('legalpersonName', result),
                    self.deal_json('regType', result),
...
if __name__ == '__main__':
    own_tax_info = OwntaxInfo()
    own_tax_info.run_main()