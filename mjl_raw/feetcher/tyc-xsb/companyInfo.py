    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            result_frame.append([
                ...
                result_dict.get('area'),
                result_dict.get('website'),
                result_dict.get('code'),
                result_dict.get('address'),
                result_dict.get('generalManager').get('id'),  # 字典
                result_dict.get('generalManager').get('cType'),
                result_dict.get('generalManager').get('name'),
                result_dict.get('companyName'),
                result_dict.get('mainBusiness'),
                result_dict.get('employeesNum'),
                result_dict.get('mobile'),
                result_dict.get('industry'),
                result_dict.get('chairman').get('id'),
                result_dict.get('chairman').get('cType'),
                result_dict.get('chairman').get('name'),
                result_dict.get('productName'),
                # result_dict.get('secretaries'),  # 字典
                result_dict.get('secretaries').get('id'),
                result_dict.get('secretaries').get('cType'),
                result_dict.get('secretaries').get('name'),
                result_dict.get('actualController'),
                result_dict.get('controllingShareholder').replace('\n', '').replace('\n\r', '').replace('\\r\\n',
                                                                                                        '').replace(
                    '\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace(
                    '，', '#'),
                result_dict.get('engName'),
                result_dict.get('registeredCapital'),
                result_dict.get('postalcode'),
                result_dict.get('legal').get('id'),
                result_dict.get('legal').get('cType'),
                result_dict.get('legal').get('name'),
                result_dict.get('name'),
                result_dict.get('fax'),
                result_dict.get('usedName'),
                result_dict.get('introduction').replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace(
                    '\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace(
                    '，', '#'),
                result_dict.get('finalController'),
                datetime.now().strftime("%Y-%m-%d")
            ])
            return result_frame
