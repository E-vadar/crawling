...
"""
@Time ： 2020/12/16 10:46 AM
@Auth ： Stone
@File ：holdingcompanyInfo.py
@IDE  ：PyCharm
"""
...
class Holding_company_Info(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "HOLDINGCOMPANY_INFO",
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('participationRatio', result),
                    self.deal_json('investmentAmount', result),
                    self.deal_json('cType', result),
                    self.deal_json('mainBusiness', result),
                    self.deal_json('name', result),
                    self.deal_json('relationship', result),
                    self.deal_json('graphId', result),
                    self.deal_json('profit', result),
                    self.deal_json('reportMerge', result),
                    datetime.now().strftime("%Y-%m-%d")
                ])
            return result_frame
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'holdingcompanyInfo.csv'
...
if __name__ == '__main__':
    holding_company_Info = Holding_company_Info()
    holding_company_Info.run_main()
    ...
    #     "profit": "-",
    #     "reportMerge": "-",
    #     "name": "深圳市振业房屋租赁运营管理有限公司",
    #     "graphId": "485568977",
    #     "mainBusiness": "",
    #     "participationRatio": "100.00%",
    #     "investmentAmount": "1000.00万",
    #     "cType": 1
    #   } 
    # print(deal_json(dict_jaja))