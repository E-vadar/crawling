...
"""
@Time ： 2020/12/16 3:11 PM
@Auth ： Stone
@File ：searchBrandAndAgency.py
@IDE  ：PyCharm
"""

# searchBrandAndAgency---搜索项目品牌/投资机构---832
# SEARCH_BRAND_AND_AGENCY(832,"searchBrandAndAgency"),
...
class SearchBrandAndAgency(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "SEARCH_BRAND_AND_AGENCY",
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('website', result),
                    self.deal_json('companyName', result),
                    self.deal_json('eventCount', result),
                    self.deal_json('type', result),
                    self.deal_json('fundCount', result),
                    self.deal_json('setupDate', result),
                    self.deal_json('round', result),
                    self.deal_json('intro', result).replace('，', '#'),
                    self.deal_json('name', result),
                    self.deal_json('logo', result),
                    self.deal_json('competingCount', result),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'searchBrandAndAgency.csv'
...
if __name__ == '__main__':
    search_brand_and_agency = SearchBrandAndAgency()
    search_brand_and_agency.run_main()