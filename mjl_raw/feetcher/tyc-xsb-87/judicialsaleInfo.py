...
"""
@Time ： 2020/12/16 5:24 PM
@Auth ： Stone
@File ：judicialsaleInfo.py
@IDE  ：PyCharm
"""

# JUDICIALSALE_INFO(850,"judicialsaleInfo"),   司法拍卖
...
class JudicialSaleInfo(object):
    ...
    # 常州市武进区银通农村小额贷款股份有限公司&pageNum=1&pageSize=10
    def get_response(self, company_name, qyxx_id=None, pageNum = 1):
        self.post_data['trdDataRequest']['name'] = "常州市武进区银通农村小额贷款股份有限公司"
        self.post_data['trdDataRequest']['pageNum'] = pageNum
        self.post_data['companyName'] = "常州市武进区银通农村小额贷款股份有限公司"
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                ...
                for deta in self.deal_json('detail', result):
                    result_frame.append([
                        ...
                        self.deal_json('scopeDate', result),
                        self.deal_json('pubTime', result),
                        # self.deal_json('detail', result),   # 列表需要解析
                        self.deal_json('jid', deta),
                        self.deal_json('initial_price', deta),
                        self.deal_json('title', deta),
                        self.deal_json('consult_price', deta),

                        self.deal_json('title', result),
                        self.deal_json('court', result),
                        self.deal_json('introduction', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'judicialsaleInfo.csv'
...
if __name__ == '__main__':
    judicial_sale_info = JudicialSaleInfo()
    judicial_sale_info.run_main()