...
"""
@Time ： 2020/12/19 4:03 PM
@Auth ： Stone
@File ：investtreeInfo.py
@IDE  ：PyCharm
"""

# 股权穿透图    INVESTTREE_INFO(455,"investtreeInfo")
...
class InvestTreeInfo(object):
    ...
    # 字典数据递归遍历，返回所有结果的key,values 集合
    def convert_list_dict(self, dict_d):
        key_list = []
        if isinstance(dict_d, dict):
            for x in range(len(dict_d)):
                temp_key = dict_d.keys()[x]
                temp_value = dict_d[temp_key]
                key_list.append(temp_key)
                self.convert_list_dict(temp_value)
        elif isinstance(dict_d, list):
            for k in dict_d:
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            self.convert_list_dict(json.loads(result_dict)),
            # print("舆情有内容，开始格式化！")
            ...
            #
            #         # self.deal_json('jigou_name', result),
            #         # self.deal_json('area', result),
            #         # self.deal_json('orgCode', result),
            #         # self.deal_json('imgPath', result),
            #         # self.deal_json('desc', result).replace('，', '#'),
            #         # self.deal_json('foundYear', result),
            #
            #         datetime.now().strftime("%Y-%m-%d")
            #     ])
            return result_frame
...
if __name__ == '__main__':
    invest_tree_info = InvestTreeInfo()
    invest_tree_info.run_main()
