    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                ...
                companylist = self.deal_json('companyList', result)
                for company in companylist:
                    result_frame.append([
                        ...
                        self.deal_json('announce_id', result),
                        self.deal_json('party1', result),
                        self.deal_json('reason', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
                        self.deal_json('bltnno', result),
                        self.deal_json('party2', result),
                        self.deal_json('bltnstate', result),
                        self.deal_json('judgephone', result),
                        self.deal_json('dealgradename', result),
                        self.deal_json('caseno', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
                        self.deal_json('bltntypename', result),
                        self.deal_json('content', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
                        self.deal_json('dealgrade', result),
                        self.deal_json('courtcode', result),
                        self.deal_json('province', result),
                        self.deal_json('bltntype', result),
                        self.deal_json('id', company),
                        self.deal_json('type', company),
                        self.deal_json('name', company),
                        self.deal_json('mobilephone', result),
                        self.deal_json('publishpage', result),
                        self.deal_json('publishdate', result),
                        self.deal_json('id', result),
                        self.deal_json('judge', result),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'courtAnnouncement.csv'
