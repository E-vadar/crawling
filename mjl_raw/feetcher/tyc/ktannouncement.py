...
import pandas as pd
import datetime
from DBHelper import MysqlHelper, HiveHelper
import random
import pyhdfs
import time

class Kt_Announcement(object):
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "KTANNOUNCEMENT_INFO",
    ...
    def parse_data(self, response, company_name, qyxx_id):
        ...
        result_frame = []
        if result_dict == '':
            print("{}--此公司没有开庭公告数量信息".format(company_name))
        else:
            # print("舆情有内容，开始格式化！")
...
if __name__ == '__main__':
    kt_announcement = Kt_Announcement()
    com_list = kt_announcement.get_company_list()

    # 定义一个空的DataFrame(), 方便读取数据累加保存
    hive_data_frame = pd.DataFrame()
    for com in com_list[0:2]:
        # 根据公司进行获取基本信息
        resp_json = kt_announcement.get_response(com[0])

        # 获取是否有数据，进行分页
        page_number = kt_announcement.is_paging(resp_json)
        if page_number != 0:
            for page in range(1, int(page_number) + 1):
                print("爬取-{}-第{}页".format(com[0], page))
                time.sleep(1)
                hive_data = kt_announcement.parse_data(kt_announcement.get_response(com[0], com[1], page), com[0], com[1])
                hive_data_frame = hive_data_frame.append(hive_data)
                # print(hive_data_frame.shape[0])
                if hive_data_frame.shape[0] >= 1:
                    # 实现文件保存CSV
                    file_name_str = kt_announcement.save_to_csv(hive_data_frame)
                    # # 实现数据写入HDFS
                    # cl_num = kt_announcement.get_hdfs_conn_upload_hdfs(file_name_str)
                    # if cl_num > 0:
                    #     print("success")
                    # else:
                    #     print("failure")
