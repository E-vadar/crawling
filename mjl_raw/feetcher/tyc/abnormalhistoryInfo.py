...
"""
@Time ： 2020/12/16 5:44 PM
@Auth ： Stone
@File ：abnormalhistoryInfo.py
@IDE  ：PyCharm
"""

# ABONRMALHISTORY_INFO(996,"abnormalhistoryInfo"),   历史经营异常
...
import pandas as pd
import pyhdfs
from db.DBHelper import HiveHelper, MysqlHelper
from util.web_request import WebRequest

class AbnormalHistoryInfo(object):
    ...
    def __init__(self):
        ...
        self.post_data = {
                "trdDataApi": "ABONRMALHISTORY_INFO",
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('putDate', result),
                    self.deal_json('removeDate', result),
                    self.deal_json('removeReason', result),
                    self.deal_json('removeDepartment', result),
                    self.deal_json('putDepartment', result),
                    self.deal_json('putReason', result).replace('\n', '').replace('\n\r', '').replace('\\r\\n', '').replace('\r', '').replace('\\n', '').replace('\\r', '').replace('\r\n|\r|\n', '').replace(',', '#').replace('，', '#'),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'abnormalhistoryInfo.csv'
...
if __name__ == '__main__':
    abnormal_history_info = AbnormalHistoryInfo()
    abnormal_history_info.run_main()
