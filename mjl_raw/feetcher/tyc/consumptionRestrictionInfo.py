...
"""
@Time ： 2020/12/19 6:15 PM
@Auth ： Stone
@File ：consumptionRestrictionInfo.py
@IDE  ：PyCharm
"""
...
class ConsumptionRestrictionInfo(object):
    ...
    def get_response(self, company_name, qyxx_id=None, pageNum = 1):
        self.post_data['trdDataRequest']['name'] = "北京百度网讯科技有限公司"
        self.post_data['trdDataRequest']['pageNum'] = pageNum
        self.post_data['companyName'] = "北京百度网讯科技有限公司"
        ...
        if result_dict == '':
            ...
            for result in json.loads(result_dict):
                result_frame.append([
                    ...
                    self.deal_json('qyinfoAlias', result),
                    self.deal_json('zhixingId', result),
                    self.deal_json('filePath', result),
                    self.deal_json('qyinfo', result),
                    self.deal_json('caseCreateTime', result),

                    self.deal_json('alias', result),    # 列表
                    self.deal_json('id', result),
                    self.deal_json('qyinfoGid', result),
                    self.deal_json('xname', result),
    ...
    def save_to_csv(self, opinion_frame):
        try:
            if opinion_frame is None:
                ...
                file_name = 'consumptionRestrictionInfo.csv'
...
if __name__ == '__main__':
    consumptionRestrictionInfo = ConsumptionRestrictionInfo()
    consumptionRestrictionInfo.run_main()