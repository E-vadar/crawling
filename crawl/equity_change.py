...
//接口名称：股本变动
//接口类型：数据获取
//接口说明：可以通过公司名称或ID获取上市公司股本变动信息，股本变动信息包括变动时间、变动原因、变动后A股总股本、变动后流通A股、变动后限售A股等
//完整URL示例：http://open.api.tianyancha.com/services/open/stock/equityChange/2.0?id=485568977&name=深圳市振业（集团）股份有限公司&keyword=深圳市振业（集团）股份有限公司&pageNum=1&pageSize=10
"""
import traceback
import requests
import json
        ...
        if (len(result)) != 0:
            for j in range(len(result)):
                ...
                myWriter.writerow(
                          ...
                          company_name,
                          do_time,
                          result[j].get('changeReason'),
                          result[j].get('changeDate'),
                          result[j].get('afterLimit'),
                          result[j].get('afterAll'),
                          result[j].get('afterNoLimit')
                           ])
            data_list_dic.clear()
    except:
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    data_list_dic = []
    with open('D:/qiyuanwork/pythonProject/parse/doc/csv_file_neeq/equity_change.csv', 'w', encoding='utf-8', newline='') as myFile:
