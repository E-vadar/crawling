        ...
        if (len(result)) != 0:
            for j in range(len(result)):
                myWriter.writerow(
                          ...
                          company_name,
                          do_time,
                          result[j].get('integrityRecordNo'),
                          result[j].get('reasons').replace('\n', '').replace(',', '#').replace('，', '#').replace('\r', ''),
                          result[j].get('implementationDepartment'),
                          result[j].get('publishValidityPeriod'),
                          result[j].get('decisionContent2').replace('\n', '').replace(',', '#').replace('，', '#').replace('\r', ''),
                          result[j].get('businessId'),
                          result[j].get('decisionContent'),
                          result[j].get('creditIcon'),
                          result[j].get('integritySubject'),
                          result[j].get('decisionDate'),
                          result[j].get('documentDumber').replace('\n', '').replace(',', '#').replace('，', '#').replace('\r', '')
                           ])
            data_list_dic.clear()
    except:
        traceback.print_exc()
...
if __name__ == '__main__':
    ...
    data_list_dic = []
    with open('D:/qiyuanwork/pythonProject/parse/doc/csv_file_neeq/bad_conduct.csv', 'w', encoding='utf-8', newline='') as myFile:
        myWriter = csv.writer(myFile)
        for com_name in range(len(data_list)):
            page_i = 1
            company_name = data_list[com_name][1]
            # company_name = '郑州久鼎路桥工程有限公司'
